(function($){
	...
	customTemplate.prototype.renderMessage = function (msgData) {
		...
		else if(msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "widgetPago"){
			try {
				localStorage.setItem('idWebhook', msgData.message[0].component.payload.id);				
				$(".chatInputBox").hide();
				messageHtml = $(this.getChatTemplate("widgetPago")).tmpl({
					'helpers': this.helpers,
					'extension': this.extension
				});
				this.bindEvents(messageHtml, msgData.message[0].component.payload.data);
			} catch (error) {
				console.log(error);
			}
		}
		else if(msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "widgetDatos"){
			try {
				messageHtml = $(this.getChatTemplate("widgetDatos")).tmpl({
					'helpers': this.helpers,
					'extension': this.extension
				});
				this.bindEvents(messageHtml);
		...
		else if(msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "datosRegistro"){
			console.log(msgData.message[0].component.payload.datos);
			
		}
		else if(msgData.message[0] && msgData.message[0].component && msgData.message[0].component.payload && msgData.message[0].component.payload.template_type == "obtenerTokenInicial" && (localStorage.getItem("token") == "undefined" || localStorage.getItem("token") == null)){
			try {
				fetch("https://crm.eluniversal.com.mx:8443/ServicesSuscripciones2/authenticate", {
						method: 'POST',
						headers: {
	...
	*/
	...
	customTemplate.prototype.getChatTemplate = function (tempType) {
		...
		*/
		...
		var widgetDatos = (datos)=>{
			let x = 
			`<div>
				<p><b>Numero de tarjeta:</b>${datos.numtarjeta}</p>
				<p><b>Fecha de vencimiento:</b>${datos.fechavencimiento}</p>
				<p><b>CVV:</b>${datos.cvv}</p>
		...
		<div>
			<div class="container">
				<div class="card-container">
					<div class="front">
						<div class="image">
							<img src="custom/image/chip.png" alt="">
							<img src="custom/image/visa.png" alt="">
						</div>
						<div class="card-number-box">Numero de tarjeta</div>
						<div class="flexbox">
							<div class="box">
								<span>Nombre</span>
				...
				<form action="">
					...
					<div class="inputBox">
						<span>Telefono</span>
						<input id="tel-input" type="tel" class="tel-input">
					</div>
					<div class="inputBox">
						<span>Correo electronico</span>
						<input id="email-input" type="email" class="email-input">
					</div>
					<button id="submit-input" class="submit-btn">Pagar</button>
				</form>
				<!-- Modal -->
						...
						<div class="modal-content">
							<div class="modal-body">
								<div class="card text-center">
									...
									<div class="card-body">
										<p>Estamos procesando tu pago en este momento.</p>
										<p>Se enviara un correo con más detalles sobre tu poliza a tu correo en cuanto sea procesado el pago.</p>
									</div>
									<button type="button" class="btn btn-primary" onclick="modalChange()">Cerrar</button>
								</div>
							</div>
						</div>
					</div>
...
//Below method is for template specific events
	...
	customTemplate.prototype.bindEvents = function (messageHtml, data) {
		chatInitialize=this.chatInitialize;
		helpers=this.helpers;
		...
		//Funcion custom template
		function getRemove(field){
			let dato = $(field).val();
			$(field).val("");
			return dato;
		}
		//funcion string aleatorio
		...
		function sendEmail(email){
			var emailDetails = {
				service_id: 'service_gw7rch4',
				template_id: 'template_t7e89i8',
				user_id: 'fsl1JfSxj_n2GNtyD',
				template_params: {
					nombre: data.nombre,
					tipoPlan: data.plan.tipoPlan,
					toMail: email,
					fechaExpiracion: data.plan.fechaFin,
					costo: data.plan.precio
				}
			};
		...
		$(messageHtml).find('.card-number-input').on('input', () =>{
			document.querySelector('.card-number-box').innerHTML = $('.card-number-input').val();
		});

		$(messageHtml).find('.card-holder-input').on('input', () =>{
			document.querySelector('.card-holder-name').innerHTML = $('.card-holder-input').val();
		});

		$(messageHtml).find('.month-input').on('input', () =>{
			document.querySelector('.exp-month').innerHTML = $('.month-input').val();
		});

		$(messageHtml).find('.year-input').on('input', () =>{
			document.querySelector('.exp-year').innerHTML = $('.year-input').val();
		});

		$(messageHtml).find('.cvv-input').on('mouseenter', () =>{
			document.querySelector('.front').style.transform = 'perspective(1000px) rotateY(-180deg)';
			document.querySelector('.back').style.transform = 'perspective(1000px) rotateY(0deg)';
		});

		$(messageHtml).find('.cvv-input').on('mouseleave', () =>{
			document.querySelector('.front').style.transform = 'perspective(1000px) rotateY(0deg)';
			document.querySelector('.back').style.transform = 'perspective(1000px) rotateY(180deg)';
		});

		$(messageHtml).find('.cvv-input').on('input', () =>{
			document.querySelector('.cvv-box').innerHTML = $('.cvv-input').val();
		});
		//fin de metodos de custom template
